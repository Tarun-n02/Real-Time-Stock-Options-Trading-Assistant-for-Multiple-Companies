...
def train_model(data):
    """
    Train a Random Forest Classifier to predict Buy/Sell/Hold signals.
    """
    loaded_model = 'trained_model.pkl'
    if not os.path.exists(loaded_model):
        print("training")
        features = ['SMA_20', 'EMA_20', 'Daily_Return', 'RSI', 'Volume']
        data['Signal'] = data['Close'].shift(-1) > data['Close']  # 1 for Buy, 0 for Sell
